...
contains
    ...
    subroutine Initialize()
        ...
        contains
            subroutine NewTrainingSet()
                logical::degenerate
                integer::index,index2,i,istate,jstate
                ...
                if(GradualFit) then
                    allocate(GeomDifference(NPoints))
                    if(Unaligned) then
                        call StandardizeGeometry_NonadabaticGradient(ReferencePointtemp.geom,ReferencePointtemp.dH,MoleculeDetail.mass,NAtoms,NStates)
                        do ip=1,NPoints
                            call StandardizeGeometry2Reference_NonadiabaticGradient(pointtemp(ip).geom,pointtemp(ip).dH,ReferencePointtemp.geom,GeomDifference(ip),MoleculeDetail.mass,NAtoms,NStates)
                        end do
                    else
                        do ip=1,NPoints
                ...
                !Convert reference point from Cartesian coordinate to internal coordinate, and transform if degenerate
                    ...
                    allocate(ReferencePoint.H(NStates,NStates))
                    allocate(ReferencePoint.dH(InternalDimension,NStates,NStates))
                    call Cartesian2Internal_Nonadiabatic(ReferencePointtemp.geom,ReferencePointtemp.dH,CartesianDimension,ReferencePoint.geom,ReferencePoint.dH,InternalDimension,NStates)
                ...
                !Convert points from Cartesian coordinate to internal coordinate, and identify almost degenerate points
                    ...
                    do ip=1,NPoints
                        ...
                        call Cartesian2Internal_Nonadiabatic(pointtemp(ip).geom,pointtemp(ip).dH,CartesianDimension,pointswap(ip).geom,pointswap(ip).dH,InternalDimension,NStates)
